...
class PrismNet1(nn.Module):
    def __init__(self, mode="pu"):
        super(PrismNet1, self).__init__()
        self.mode = mode
        h_p, h_k = 2, 5 
        if mode=="pu":
            self.n_features = 5
...
class PrismNet2(nn.Module):
    def __init__(self, mode="pu"):
        super(PrismNet2, self).__init__()
        ...
        self.conv    = Conv2d(1, base_channel, kernel_size=(11, h_k), bn = True, same_padding=True)
        self.se      = SEBlock(base_channel)
        self.res2d   = ResidualBlock2D(base_channel, kernel_size=(11, h_k), padding=(self.n_features, h_p)) 
        self.res1d   = ResidualBlock1D(base_channel*4) 
        self.avgpool = nn.AvgPool2d((1,self.n_features))
        self.gpool   = nn.AdaptiveAvgPool1d(1)
        self.fc      = nn.Linear(base_channel*4*8, 1)
...
class PrismNet0(nn.Module):
    def __init__(self, n_features=5):
        super(PrismNet0, self).__init__()
        assert n_features in [1,4,5], "Wrong num of features: {}".format(n_features)
        h_p, h_k = 2, 5 
        if n_features == 4:
            h_p, h_k = 1, 3 
